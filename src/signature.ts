 ...
 *
 * Done in code rather than asked of the model: a trailer must be byte-exact for
 * git and GitHub to parse it, and a model will paraphrase names, mistype
 * addresses, or drop the line entirely on a long diff.
 */
export function appendSignature(message: string, lines: string[], vars: TemplateVars): string {
	...
	const trailers = lines
		...
		// A variable that resolves to nothing leaves a bare "Refs:" — drop it
